...
A series of connected and sequential exercises will be given. These exercises are based on the dataset `shapes.xlsx` and contains six different datasets stored as different worksheets

To make yourself familiar with a number of deterministic functions, you are asked to take a number of steps:

1. Read in a dataset into `R`. We recommend using the `openxls` package which allows you to specify the worksheet you want to read in. For example, for the worksheet of the `shapes.xlsx` file, we can read it into R with the following:
...
2. make plots of the first two datasets and choose at least two appropriate functions for each dataset.

3. Next, you will explore the properties of the selected functions after which you will choose appropriate parameter values through eyeballing so you get a reasonable fit between the data and the choosen function.
  ...
  + _hint 5_: dataset 6 you need to figure out for yourselves ;)
  + _hint 6_: (optional) dataset 6 describes a species response curve (presence/absence). Fit a model that models the probability of presence (use google to find a good one).

The following exercise is a must; each written exam contains a question that is like it.

:::::: {.callout-important title="Exercise" collapse="true"}

1. Choose (at least) two appropriate functions based on the type of data or the shape of the data. If you have time please repeat this for all datasets.
...
**dataset 1**: There are a number of options of functions to choose from, depending on the level of sophistication:

$\frac{ax}{(b+x)}$, $a(1-e^{(-bx)})$, $\frac{1}{2\theta}(\alpha I+p_{max}-\sqrt(\alpha I+p_{max})^2-4\theta I p_{max})$ see page 98 of Bolker.

**dataset 2**: Bolker mentions four functional responses of which the following would be appropiate $min(ax,s)$ $\frac{ax}{(b+x)}$, $\frac{ax^2}{(b^2+x^2)}$,$\frac{ax^2}{(b+cx+x^2)}$
...
2. Check chapter 3 from the book
...
Choose appropriate parameter values through eyeballing so that the chosen curves more or less match the data. Eyeballing means that you knowledge on the effect of parameter values on the shape of the function (see question 4). Later we will use likelihood methods to estimate the parameter values. Plot the curves on top of the data using `curve`.

::::: {.content-hidden unless-meta="show_solution"}

:::: {.callout-tip title="Solution" collapse="true"}

Check the plotted curve against the data to see how well the eye-balled parameters work
...
implementing functions with AD support. The two most popular are the Template Model Builder
(TMB for short) and Stan. These packages are used for more advanced models (the kind introduced
in chapters 10 and 11 of the book).
