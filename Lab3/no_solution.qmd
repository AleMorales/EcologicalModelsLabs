...
**dataset 2**: Bolker mentions four functuonal responses of which the following would be appropiate $min(ax,s)$ $\frac{ax}{(b+x)}$, $\frac{ax^2}{(b^2+x^2)}$,$\frac{ax^2}{(b+cx+x^2)}$
...
2. See the word file on Brightspace "Bestiary of functions.docx"
...
Note that the print statement was added to print to screen `log(c(1:10))`
